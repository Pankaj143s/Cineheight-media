...
export default function PhoneReelExperience({
  ...
  /** Measure the stage and derive phone size + spacing from its height. */
  ...
  const applyVisuals = useCallback(() => {
    ...
    for (let i = 0; i < cardRefs.current.length; i++) {
      ...
      const rotate = clamp(-off * (mobile ? 9 : 15), -34, 34)
      const z = -abs * (mobile ? 60 : 130)
      el.style.transform =
  ...
  return (
    <section aria-label={label} className="relative">
      {/* The heading sits behind the phones on desktop — one composition, not
          a header block with a carousel underneath. */}
      {mobile ? (
        <div className="flow-gutter relative z-10">{headingBlock}</div>
      ) : (
        <div className="pointer-events-none absolute inset-x-0 top-0 z-0 flow-gutter" style={{ paddingTop: '2vh' }}>
          {headingBlock}
        </div>
      )}
      ...
      {/* Active label + controls — reads as the installation's own base plate */}
      <div className="flow-gutter relative z-10 mt-8 flex flex-wrap items-center justify-between gap-x-8 gap-y-5">
        ...
        <div className="flex items-center gap-4">
          <div className="hidden h-px w-24 overflow-hidden bg-white/10 sm:block" aria-hidden="true">
