...
| H-01 | Client Stories lower-third controls collide with film content | High | 1 | `/` | `components/home/ClientStories.tsx`, story metadata | Pending | Build mobile safe-zone attribution and touch-control rails while preserving full-bleed film and desktop behavior. | Screenshots at 320/360/390/430; every story checked; reduced motion and touch controls pass. | Pending | None |
| H-02 | Featured Sapale film loads early and desktop media can reach mobile | High | 1 + 3 | `/` | `components/home/FeaturedWorkJourney.tsx`, `content/presentationMedia.ts`, `content/mediaSlots.ts`, generated derivatives | Pending | Gate source/playback by proximity and add responsive source contract; generate safe derivatives from masters where possible. | Cold network request order, no mount playback, no double request, mobile/desktop source dimensions and codecs verified. | Pending | Asset generation depends on available masters and ffmpeg, both to be verified. |
| M-01 | Capability taxonomy clips on narrow phones | Medium | 1 | `/` | `components/home/HomeCapabilities.tsx`, `app/globals.css` | Pending | Use bounded compact grid/insets without hiding content or shrinking type; retain video and line choreography. | 320/360/390/430 and 200% text zoom; touch feedback and reduced motion pass. | Pending | `app/globals.css` contains user changes; implementation hunks must be isolated. |
| M-11 | Missing skip link and stable main-content target | Medium | 1 | Site-wide | `app/layout.tsx`, page roots | Pending | Add one root skip link and exactly one stable target per rendered route. | Keyboard-first focus, fixed-nav offset, no duplicate IDs, route transition compatibility. | Pending | None |
| M-12 | Invalid contact submission does not focus first invalid field | Medium | 1 | `/contact`, compact form | `components/contact/ProjectContactForm.tsx` | Pending | Ordered refs, fixed-nav-aware focus/scroll, refined pulse, reduced-motion guard. | Keyboard and screen-reader semantics; first error focus; existing live status and descriptions preserved. | Pending | None |
| L-07 | Desktop Home navigation target is too small | Low | 1 | Global navigation | `components/Navbar.tsx` | Pending | Increase invisible interaction area without pill styling or visual bloat. | Fine-pointer geometry and keyboard focus; mobile 44 px controls unchanged. | Pending | None |
| L-08 | Combined email/phone field has email-only semantics | Low | 1 | `/contact`, compact form | `components/contact/ProjectContactForm.tsx` | Pending | Prefer an explicit contact-method control with matching input semantics and validation. | Keyboard order, autofill attributes, input mode, live method change, reduced motion. | Pending | None |
